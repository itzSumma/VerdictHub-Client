...
export default function SignInPage() {
  return (
    <Suspense fallback={<AuthLoader text="Preparing secure sign in..." />}>
      <SignInForm />
    </Suspense>
  );
}
...
function SignInForm() {
  ...
  const safeRedirect = redirectTo === "/" ? "/dashboard" : redirectTo;
  ...
  return (
    <AuthShell
      ...
      title="Welcome back to VerdictHub"
      subtitle="Sign in to manage cases, payments, legal profiles, and dashboard activity from one polished workspace."
    >
      <form onSubmit={onSubmit} className="space-y-4">
        <AuthInput label="Email address" name="email" type="email" placeholder="you@example.com" autoComplete="email" />
        <AuthInput label="Password" name="password" type="password" placeholder="Enter your password" autoComplete="current-password" />
        ...
        <p className="text-center text-sm text-slate-500">
          New to VerdictHub?{" "}
          <Link href="/signup" className="font-black text-amber-700 hover:text-amber-800">
            Create an account
          </Link>
        </p>
      </form>
    ...
    <section className="relative isolate overflow-hidden py-12 sm:py-16">
      ...
      <div className="mx-auto grid max-w-6xl items-center gap-8 px-4 lg:grid-cols-[0.95fr_1.05fr]">
        <div className="hidden rounded-[2rem] bg-slate-950 p-8 text-white shadow-2xl shadow-slate-950/20 lg:block">
          ...
          <h2 className="mt-8 text-4xl font-black leading-tight">Your legal dashboard, ready the moment you sign in.</h2>
          <p className="mt-5 leading-8 text-slate-300">Track hires, publish services, review payments, and keep every legal workflow moving without clutter.</p>
          <div className="mt-8 grid gap-3 text-sm font-bold text-slate-200">
            {["Role-based dashboard", "Protected JWT sessions", "Secure payment workflow"].map((item) => (
        ...
        <div className="premium-card mx-auto w-full max-w-xl rounded-[2rem] p-6 sm:p-8">
          <div className="mb-7">
            <p className="text-xs font-black uppercase tracking-[0.3em] text-amber-700">{eyebrow}</p>
