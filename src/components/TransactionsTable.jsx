...
export default function TransactionsTable() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      const response = await authorizedFetch("/transactions/me", data?.token);
  ...
  return (
    <div className="mt-6 overflow-x-auto rounded-2xl border bg-white">
      <table className="w-full text-left text-sm">
        <thead className="bg-slate-50 text-slate-500"><tr><th className="p-4">Transaction ID</th><th className="p-4">Client</th><th className="p-4">Lawyer</th><th className="p-4">Amount</th><th className="p-4">Date</th><th className="p-4">Status</th></tr></thead>
        <tbody>
